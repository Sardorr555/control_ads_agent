...
- [ ] **TASK-4.4:** Разработать нагрузочный тест изоляции процессов `tests/test_process_isolation_stress.py`:
  ...
  * Замер влияния на порт 5000:
    ...
    2. Уровень ошибок платежей: строго **0.00%**.
    3. Потребление памяти процессом трекинга $\le 200\text{ МБ RSS}$.
  * *Критерий верификации:* `pytest tests/test_process_isolation_stress.py -v` (зелёный).

### 🛑 КОНТРОЛЬНАЯ ТОЧКА В (Checkpoint C — Допуск к Gate 2):
> **Отчёт человеку:** Предъявить результаты стресс-теста изоляции процессов с подтверждением числовых порогов latency, error rate и памяти.

---

### Блок 5: Движок сквозной атрибуции и метрик (Risk Order: 5)
- [ ] **TASK-5.1:** Реализовать клиент к БД `src/storage/mysql_readonly_client.py`:
...
- [ ] **TASK-5.2:** Реализовать движок сопоставления сессий `src/core/matcher.py`:
...
- [ ] **TASK-5.3:** Реализовать модуль финансовой аналитики `src/core/metrics_engine.py`:
...
- [ ] **TASK-5.4:** Доработать `src/core/retention_guard.py`:
  * Ежесуточное агрегирование сырых событий в витрину `daily_attribution_summary` перед удалением.
- [ ] **TASK-5.5:** Написать Unit-тесты `tests/test_attribution_models.py` и `tests/test_metrics_decimal_precision.py`:
